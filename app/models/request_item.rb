class RequestItem < ApplicationRecord
  ...
  validates :quantity_loan, presence: true, :numericality => {:only_integer => true, :greater_than => 0}, :allow_nil => true
  validates :quantity_disburse, presence: true, :numericality => {:only_integer => true, :greater_than => 0}, :allow_nil => true
  validates :quantity_return, presence: true, :numericality => {:only_integer => true, :greater_than => 0}, :allow_nil => true
  ...
  def request_type_quantity_validation
    case self[:request_type]
      when 'disbursement'
        errors.add(:quantity_disburse, "Must be greater than 0") unless quantity_disburse > 0
      when 'loan'
        errors.add(:quantity_loan, "Must be greater than 0") unless quantity_loan > 0
        errors.add(:quantity_return, "Must be greater than 0") unless quantity_return > 0
      else # when 'mixed'
        errors.add(:quantity_disburse, "Must be greater than 0") unless quantity_disburse > 0
        errors.add(:quantity_loan, "Must be greater than 0") unless quantity_loan > 0
        errors.add(:quantity_return, "Must be greater than 0") unless quantity_return > 0
